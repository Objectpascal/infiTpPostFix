...
var
  i,indexBefor,indexAfter,lastpos:integer;
  OpStr,subStringAfter,strPrentasess:string;
  OpName:TopNames;
begin
{ TODO :
                          ...
                          begin
                            ...
                            if pos(',',subStringAfter)>0 then
                              begin
                                ...
                                lastpos:=2;
                                while (lastpos<=length(subStringAfter))
                                do
                                begin
                                   if TOperation.isOpenParentheses(subStringAfter[lastpos]) then
                                      strPrentasess:=strPrentasess+'('
                                   else
                                    if ( TOperation.isCloseParentheses(subStringAfter[lastpos]) and ( length(strPrentasess)>0)  ) then
                                      begin
                                        Delete(strPrentasess,length(strPrentasess),1);
                                      end;

                                     if ( (length(strPrentasess)=0) and   ( subStringAfter[lastpos] in [',']))  then
                                     begin
                                    ...
                                    lastpos:=lastpos+1;
                                      ...
                                      insert('[',FExp,i+2);
                                      insert(']',FExp,i+lastpos+1);
                                      insert('[',FExp,i+lastpos+3);
